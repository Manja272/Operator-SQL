...
-- select to_char(sysdate, 'day') from dual;   //Using the character variable to display the today Name-- select * from hr.employees ;
...
--     SELECT department_id, SUM(salary) AS total_salary, AVG(salary) AS avg_salary, MIN(salary) AS min_salary,
--        MAX(salary) AS max_salary, STDDEV(salary) AS salary_stddev, VARIANCE(salary) AS salary_variance
...
-- from hr.employees group by department_id having sum(salary)>100000 order by totalsales asc
